...
trait SpatialTestTables { self: SpatialHiveTestEnvironment =>
  // spatial/src/test/resources/polygons.csv when JVM is not forked
  def uriCSV: String     = new File("../spatial/src/test/resources/polygons.csv").toURI.toString
  def uriParquet: String = new File("../spatial/src/test/resources/polygons.snappy.parquet").toURI.toString
  ...
  def createViews(): Unit =
    ssc.sql(
      """
        ...
        |  SELECT *, ST_GeomFromWKT(wkt) AS geom FROM polygons_csv
        |);
        |""".stripMargin
    )
