...
/**
 * Source: https://github.com/locationtech/rasterframes/blob/0.10.1/core/src/main/scala/org/locationtech/rasterframes/encoders/SerializersCache.scala
 */
object SerializersCache {

  /**
   * Spark partitions are executed on a blocking thread pool. We can keep the cache of (De)Serializers (every serializer instance creation is pretty
   * expensive), but the cache should be local per thread.
   *
   * When used from multiple threads (De)Serializers tend to corrupt data and / or fail at runtime. The alternative can be to use global locks or to
   * use a separate executor per each (De)Serializer.
   */
   ...
   */
  def rowDeserialize[T](implicit tag: TypeTag[T], encoder: ExpressionEncoder[T]): Row => T = { row =>
    deserializer[T](tag, encoder)(rowSerializer[T](tag, encoder)(row))
  }

  def rowSerialize[T](implicit tag: TypeTag[T], encoder: ExpressionEncoder[T]): T => Row = { t =>
